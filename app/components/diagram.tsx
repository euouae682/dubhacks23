...
export function Diagram() {
  ...
  const toggleSelected = (e: any) => {
    e.target.key === selectedPoint ? setSelectedPoint(0) : setSelectedPoint(e.target.key);
  }
      ...
      <div className="fixed w-[32%] h-full m-auto bg-[url('/humandiagram.png')] bg-no-repeat bg-top bg-[length:100%]">
        <ul className="h-full">
          {healthPoints.map(({ id, location, title, details, color }) => {
            return (
              <>
              <li key={id} style={{top: LOCATIONS[location][0] + '%', left: LOCATIONS[location][1] + '%', backgroundColor: color}} onClick={toggleSelected}
              ...
              <div style={{display: selectedPoint !== 0 ? 'block' : 'none'}} className="absolute bg-orange-100 rounded-2xl w-1/3 h-1/6 top-10 right-5 text-center drop-shadow-lg">
