            ...
            <div className="relative w-3/5 h-full m-auto bg-[url('/TEMP.png')] bg-no-repeat bg-contain">
                <ul className="relative h-full">
                    { healthPoints.map(({ topPos, leftPos, status}) => {
                        return (<li className={"absolute block top-[" + topPos + "%] left-[" + leftPos + "%] w-5 h-5 -ml-2.5 -mt-2.5 " + (status === 'caution' ? 'bg-yellow-500' : 'bg-red-600') + " rounded-3xl drop-shadow cursor-pointer"}></li>)
                    })}
