...
export function WeekTable({ selectedDate, onClickDay }: WeekTableProps) {
  ...
  const initializeDateRange = () => {
    const firstDate: Date = new Date(
      ...
      curDateTime.getDate() - curDateTime.getDay()
    );
    ...
    <div className="bg-orange-200 rounded-3xl p-5">
      <header className="flex justify-between mb-5 text-orange-900">
        ...
        <div className="flex gap-5 items-center">
          ...
          <p>
            {dateRange[0].toDateString().slice(4, 10)} -
            {" " + dateRange[1].toDateString().slice(4, 10)}
          </p>
      ...
      <div className="grid grid-cols-7 gap-5 text-lg text-center text-white">
        {[
          ...
          new Date(
            dateRange[0].getFullYear(),
            dateRange[0].getMonth(),
            dateRange[0].getDate() + 2
          ),
          new Date(
            dateRange[0].getFullYear(),
            dateRange[0].getMonth(),
            dateRange[0].getDate() + 3
          ),
          new Date(
            dateRange[1].getFullYear(),
            dateRange[1].getMonth(),
            dateRange[1].getDate() - 2
          ),
          new Date(
            dateRange[1].getFullYear(),
            dateRange[1].getMonth(),
            dateRange[1].getDate() - 1
          ),
          dateRange[1],
        ].map((date) => {
          return (
