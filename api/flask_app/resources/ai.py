...
def create_prompt(events):
    ...
    prompt += 'Fill in the rest of the schedule with tasks the patient can do by themselves to aid with recovery. Be specific, and do not include things the patient would do anyway, such as sleep. Your answer should be formatted as a list of "<task_title>, <location>, <start_time>, <end_time>" with a linebreak in between each task.'
    return prompt

def get_bard_response(prompt) -> str:

    vertexai.init(project=current_app.config['GOOGLE_PROJECT_ID'])
    parameters = {"temperature": .8,  "max_output_tokens": 256, "top_p": 0.8, "top_k": 40}
    ...
    def post(self, date, id):
        ...
        if events:
            ...
            for new_event in response[1:].split('\n'):
                ...
                new_event = new_event[:2] + new_event[2].split(' - ')
            ...
            return new_events
            # return {'recommendation': extract_message(openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=[
